...
- [x] Observability profile: Prometheus + blackbox + docker-stats exporter + Loki/Alloy + Grafana, two provisioned dashboards (overview incl. TTFT and spend from Postgres; RAG service), ten alert rules routed through a small relay to phone notifications (ntfy/Slack/Discord); firing and resolved delivery verified end to end; login-time agent starts colima and the stack after a reboot; probe targets are a file, so any other app on the machine (containers or host processes) gets an up/down tile and an alert in one line; host disk/memory/swap/load exported from macOS
